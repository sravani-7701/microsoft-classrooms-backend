...
var storage = new GridFsStorage({
  ...
  router.post("/upload/:id",upload.single('file'), async(req, res) => {
    ...
    for(var i=0;i<length;i++){
      const user= await User.findOne({email:team.StudentEmail[i]});
      var activityType="fileadded"
      user.Activity.push({activityType,teamName:team.name})
      await  user.save();
      pusher.trigger("channel","fileadded",{
    ...
    if(!req.file||!id||!startDate||!endDate){
      ...
      if(!team) return res.status(403).json({message:"Team not found"});
        ...
        team.Assignments.push({filename: req.file.filename, originalname: req.file.originalname,Studentanswers:[],
          Scheduleddate:startDate,Enddate:endDate});
       await team.save();
       var length=team.StudentEmail.length;
       for(var i=0;i<length;i++){

         const user= await User.findOne({email:team.StudentEmail[i]});
         var activityType="assignment"
         user.Activity.push({activityType,teamName:team.name})
         await user.save();
         pusher.trigger("channel","assignment",{
